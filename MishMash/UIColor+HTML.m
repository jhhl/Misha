...
@implementation UIColor (HTML)
 NSUInteger integerValueFromHex(NSString *  hexS)
{
    unsigned int result = 0;
    sscanf([hexS UTF8String], "%x", &result);
    return result;
}

/**
 @parameter: XXXXXX or XXX
 always alpha 1.0
 */
...
- (UIColor *)darkenBy:(CGFloat) darkness
{
    ...
    {
        // grayscale
        CGFloat white = MIN(1.0,components[0]*darkness);
        CGFloat alpha = components[1];
    ...
    {
        // grayscale
        CGFloat red = MIN(1.0,components[0]*darkness);
        CGFloat green = MIN(1.0,components[1]*darkness);
        CGFloat blue = MIN(1.0,components[2]*darkness);
        CGFloat alpha = components[3];
...
CGFloat forwlerp(CGFloat t, CGFloat lo,CGFloat hi);
CGFloat forwlerp(CGFloat t, CGFloat lo,CGFloat hi)
{
   
       return fmod(t*fmod((hi-lo)+1.0,1.0)+lo ,1.0);
}
...
+ (NSArray *) arrayOfN:(int) n colorsFrom:(UIColor *) fromColor to:(UIColor * ) toColor
{
    ...
    [toColor getHue:&tc[0] saturation:&tc[1]  brightness:&tc[2]  alpha:&tc[3]] ;
    ...
    for(int i=0; i< n;i++)
    {
        ...
        ff[0] = forwlerp(t,fc[0],tc[0]);
        ff[1] = forwlerp(t,fc[1],tc[1]);
        ff[2] = forwlerp(t,fc[2],tc[2]);
        ff[3] = forwlerp(t,fc[3],tc[3]);
      
        UIColor *c = [UIColor colorWithHue:ff[0] saturation:ff[1] brightness:ff[2] alpha:ff[3]];
        [colors addObject:c];
    }
